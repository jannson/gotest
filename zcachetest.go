...
func mainImage() {
	...
	cache.SetCache("helloaaa", "myhello")
	fmt.Printf("the result is %v", cache.Exist("helloaaa"))
	var tests string
	tests, _ = cache.FindCache("helloaaa")
	fmt.Printf("the value is %v", tests)

	var fname string
	var data []byte
	fname = "/home/janson/download/testimg/1.jpeg"
	...
	md5Sum, w, h, g, x, y, f, imgType = "testmd5lalala", 500, 400, 0, -1, -1, "webp", ""
	s = 0
	...
	ret = mw.ReadImageBlob(data)
	if ret != nil {
		fmt.Printf("cannot read image")
		return
	}

	ret = convert(mw, request)
	if ret != nil {
		fmt.Printf("cannot convert image")
		return
	}

	data = mw.GetImageBlob()
	if data == nil || len(data) == 0 {
		fmt.Printf("canot got image")
	}

	ret = ioutil.WriteFile("./test.webp", data, 0644)
	if ret != nil {
		fmt.Printf("write file error")
	}

	cache.SetCacheBin(request.Md5, data)
}
